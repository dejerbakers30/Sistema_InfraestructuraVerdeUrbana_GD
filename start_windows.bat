...
start "Gemelo Digital - Backend" cmd /k "cd /d "%~dp0backend" && venv\Scripts\python.exe -m uvicorn app.main:app --reload --host 0.0.0.0 --port 8000"

echo [2/2] Iniciando servidor Frontend (Next.js)...
start "Gemelo Digital - Frontend" cmd /k "cd /d "%~dp0frontend" && npm run dev"
