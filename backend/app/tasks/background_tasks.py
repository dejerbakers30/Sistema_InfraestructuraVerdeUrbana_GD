...
from app.services.envi_met_orchestrator import EnviMetOrchestrator
...
async def _run_simulation_async(task_instance, simulation_id: str) -> dict:
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            orchestrator = EnviMetOrchestrator()
            
            # Generate input files
            input_path = orchestrator.generate_input_files(
                simulation_id=simulation_id,
                scenario_config=simulation.scenario.vegetation_config if simulation.scenario else {}
            )
...
def run_simulation_task(self, simulation_id: str) -> dict:
    ...
    return asyncio.run(_run_simulation_async(self, simulation_id))


async def _generate_report_async(task_instance, report_id: str) -> dict:
...
def generate_report_task(self, report_id: str) -> dict:
    ...
    return asyncio.run(_generate_report_async(self, report_id))
...
async def _update_simulation_progress_async(simulation_id: str, progress: int) -> dict:
...
def update_simulation_progress_task(simulation_id: str, progress: int) -> dict:
    ...
    return asyncio.run(_update_simulation_progress_async(simulation_id, progress))
