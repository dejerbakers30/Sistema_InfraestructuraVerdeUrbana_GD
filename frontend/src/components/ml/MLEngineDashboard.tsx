...
export default function MLEngineDashboard() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 bg-slate-900/60 border border-slate-800 rounded-2xl backdrop-blur-xl">
        <div>
          <div className="flex items-center gap-3">
            <span className="px-3 py-1 rounded-full text-xs font-bold bg-teal-500/10 text-teal-400 border border-teal-500/20">
              Motor de ML v2.0
            </span>
            <h2 className="text-xl font-bold text-white">Predicción Microclimática y Pruebas Estadísticas</h2>
          </div>
        ...
        <button
          ...
          className="px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 text-sm font-semibold flex items-center gap-2 transition-all shadow-md"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-6 bg-slate-900/50 border border-slate-800 rounded-2xl">
          ...
          <form onSubmit={handleUpload} className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-slate-400 mb-2">Selecciona un archivo urbano (.csv o .geojson)</label>
              <input
                type="file"
                accept=".csv, .geojson, .json"
                onChange={e => setFile(e.target.files?.[0] || null)}
                className="w-full text-xs text-slate-400 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-slate-800 file:text-teal-400 hover:file:bg-slate-700 cursor-pointer border border-slate-800 rounded-xl bg-slate-950 p-2"
              />
            </div>
            <button
              type="submit"
        ...
        <div className="p-6 bg-slate-900/50 border border-slate-800 rounded-2xl">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-slate-400 mb-1">Dataset Seleccionado</label>
              <select
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-400 mb-1">Variable Microclimática Objetivo</label>
              <select
      ...
      {models.length > 0 && (
        <div className="p-6 bg-slate-900/50 border border-slate-800 rounded-2xl">
          <h3 className="text-lg font-bold text-white mb-4 flex items-center justify-between">
            ...
            <span className="text-xs font-normal text-slate-400">Validación Cruzada 5-Fold</span>
          </h3>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-200">
                {models.map(m => (
                  <tr key={m.id} className={`hover:bg-slate-800/40 transition-colors ${m.is_active ? 'bg-teal-500/10' : ''}`}>
                    <td className="py-3.5 px-4 font-semibold text-white flex items-center gap-2">
                      {m.is_active && <span className="w-2 h-2 rounded-full bg-teal-400 animate-pulse" />}
                      {m.name}
                    </td>
                    <td className="py-3.5 px-4 font-mono text-teal-400">{m.metrics?.rmse?.toFixed(4)}</td>
                    <td className="py-3.5 px-4 font-mono">{m.metrics?.mae?.toFixed(4)}</td>
                    <td className="py-3.5 px-4 font-mono text-emerald-400">{m.metrics?.r2?.toFixed(4)}</td>
        ...
        <div className="p-6 bg-slate-900/50 border border-slate-800 rounded-2xl space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 bg-slate-950/60 border border-slate-800 rounded-xl">
              <span className="text-xs text-slate-400 font-semibold">Test Global de Friedman</span>
              <p className="text-xl font-bold text-white mt-1">F = {statsData.friedman_statistic?.toFixed(4)}</p>
              <p className="text-xs text-indigo-400 font-mono mt-0.5">p-valor: {statsData.friedman_p_value?.toExponential(3)}</p>
            ...
            <div className="p-4 bg-slate-950/60 border border-slate-800 rounded-xl">
              <span className="text-xs text-slate-400 font-semibold">Diferencia Crítica Nemenyi (CD)</span>
              <p className="text-xl font-bold text-teal-400 mt-1">CD = {statsData.nemenyi_results?.critical_difference?.toFixed(4)}</p>
              <p className="text-xs text-slate-400 mt-1">Margen de separación entre rangos de modelos</p>
            </div>

            <div className="p-4 bg-slate-950/60 border border-slate-800 rounded-xl">
              <span className="text-xs text-slate-400 font-semibold">Rangos Promedio de Error</span>
              <div className="mt-2 space-y-1">
                {Object.entries(statsData.nemenyi_results?.average_ranks || {}).map(([name, rank]) => (
                  <div key={name} className="flex justify-between text-xs font-mono">
                    ...
                    <span className="text-teal-400 font-bold">{rank}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Conclusion Text Box */}
          <div className="p-4 bg-slate-950/80 border border-indigo-500/20 rounded-xl">
            <h4 className="text-xs font-bold text-indigo-300 uppercase tracking-wider mb-1">Conclusión Científica Automatizada</h4>
            <p className="text-sm text-slate-300 leading-relaxed">{statsData.conclusion_text}</p>
          </div>
        </div>
      )}
